...
      {showCommunityModal && (<CommunityModal allCards={allCards} onClose={() => setShowCommunityModal(false)} onLoadDeck={(d, n) => { const mainCards = d.m.map(id => allCards.find(c => c.id === id)).filter(Boolean); const extraCards = d.e.map(id => allCards.find(c => c.id === id)).filter(Boolean); setDeck({ main: mainCards, extra: extraCards }); setDeckName(n); setToastMsg("社群牌組載入成功！"); }} user={user} />)}
...
                                <button onClick={() => setShowBulkModal(true)} className="bg-green-600 hover:bg-green-700 text-white p-2 rounded-lg text-sm font-bold flex items-center gap-1 shadow transition-colors"><FileJson size={16} /></button>
                            </div>
                        )}
                    </div>
                </div>
