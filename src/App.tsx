...
export default function App() {
  ...
  useEffect(() => {
    ...
    }, (error) => { setIsDataLoaded(true); if (error.code === 'permission-denied') setToastMsg("權限不足：無法讀取卡片資料"); else setToastMsg(`資料庫讀取失敗: ${error.message}`); });
    return () => unsubscribe();
  }, [user]);
  ...
  const handleLoadDeckFromCommunity = (loadedDeckObj, newName) => { if (deck.main.length > 0 || deck.extra.length > 0) { if (!confirm(lang==='en'?"Overwrite current deck?":"目前的牌組將被清空覆蓋，確定載入？")) return; } setDeck({ main: loadedDeckObj?.main || [], extra: loadedDeckObj?.extra || [] }); setDeckName(newName || "社群牌組"); setToastMsg("✨ 社群牌組載入成功！"); setShowCommunityModal(false); };
