...
<head>
    ...
    <title>CookieRun: Braverse Deck Builder</title>

    <!-- 1. Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>

    <!-- 2. Html2Canvas (預先載入，確保輸出圖片功能順暢) -->
    <script src="https://html2canvas.hertzen.com/dist/html2canvas.min.js"></script>

    <!-- 3. Import Map: 這是讓瀏覽器看懂 'import ... from "react"' 的關鍵 -->
...
<body>
    ...
    <div id="root">
        <!-- 初始載入畫面 -->
        <div class="flex h-screen flex-col items-center justify-center bg-slate-50 text-slate-600">
            <div class="loader-spin h-10 w-10 border-4 border-blue-200 border-t-blue-600 rounded-full mb-4"></div>
            <p class="font-bold animate-pulse">正在啟動牌組編輯器...</p>
        </div>
    </div>

    <!-- 這裡使用 type="text/babel" 加上 data-type="module" 讓 Babel 處理 import -->
    <script type="text/babel" data-type="module">
        import React, { useState, useMemo, useEffect, useRef } from "react";
        import { createRoot } from "react-dom/client"; // 額外加入這行以支援 React 18 Mounting
        import {
            Search, Filter, Trash2, AlertCircle, Layers, Box, Zap, AlertTriangle,
            Palette, RotateCw, Plus, X, Image as ImageIcon, Upload, Eye, Share2,
            Download, Link as LinkIcon, Copy, Database, Cloud, Lock, RefreshCw,
            Pencil, Star, Youtube, FileJson, WifiOff, CheckCircle, Cookie
        } from "lucide-react";
        ...
        import {
            getFirestore, collection, doc, setDoc, onSnapshot, query, writeBatch, deleteDoc
            ...
            const sampleFormat = `[
  {
    ...
    "isSoulJam": false
        ...
        const ExportModal = ({ deck, deckName, onClose }) => {
            ...
            useEffect(() => {
                // html2canvas 已在 head 引入，這裡保留檢查邏輯
                        ...
                        <div className={`flex-1 ${compact ? "" : ""}`}>
                                ...
                                >
                                    {!compact && (
                                        <button
                                            ...
                                            className="p-1 text-current opacity-0 group-hover:opacity-100 hover:bg-white/50 rounded-full transition-all"
                                            title="檢視詳細大圖"
                                        >
                                            <Eye size={16} />
                                        </button>
                            ...
                            {!compact && (
                                <div className="flex flex-wrap items-center gap-1.5 text-xs opacity-90 font-semibold">
                                    <span className="text-[10px] font-bold border border-current px-1 rounded opacity-80 uppercase bg-white/30">
                                        {card.color}
                                    </span>
                                    ...
                                    {card.level && (
                                        <span className="text-[10px] font-bold bg-yellow-400 text-yellow-900 px-1 rounded shadow-sm">
                                            {card.level}
                                        </span>
                                    )}
                                    {card.isFlip && (
                                        <span className="flex items-center gap-0.5 text-[10px] bg-slate-800 text-white px-1.5 rounded font-bold tracking-wider">
                                            FLIP
                                        </span>
                                    )}
                                    {card.isExtra && (
                                        <span className="text-[10px] uppercase tracking-wider bg-purple-200 text-purple-900 px-1 rounded border border-purple-300">
                                            EXTRA
                                        </span>
                                    )}
                    ...
                    <div className="flex-1 flex flex-col min-w-0 border-r border-slate-200">
                        <div className="p-4 bg-white border-b border-slate-200 shadow-sm z-10 space-y-3">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                {isOffline && (
                                    <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-3 py-1.5 rounded text-xs flex items-center gap-2 mt-1">
                                        <WifiOff size={14} />
