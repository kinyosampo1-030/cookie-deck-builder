        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-100">
          {activeTab === "image" ? (
            <div className="flex flex-col items-center gap-6">
              <button onClick={handleDownloadImage} disabled={isGenerating} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-bold text-xl flex items-center gap-2 disabled:opacity-50">
                {isGenerating ? "生成中..." : <><Download size={24} /> 下載圖片</>}
              </button>
              <div ref={exportRef} className="bg-white p-10 rounded-lg shadow-lg w-full max-w-[1000px] min-h-[600px] border border-slate-200">
                <div className="flex justify-between items-end border-b-4 border-slate-800 pb-6 mb-8">
                  <div>
                    <h1 className="text-4xl font-bold text-slate-900 uppercase">{deckName || "My Deck"}</h1>
                    <p className="text-slate-500 mt-2 text-xl font-bold">Total Cards: {deck.main.length + deck.extra.length}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-base font-bold text-slate-400">CREATED WITH</div>
                    <div className="text-2xl font-black text-blue-600">Braverse Deck Builder</div>
                  </div>
                </div>
                <div className="mb-10">
            ...
            <div className="flex flex-col gap-6 max-w-2xl mx-auto">
               <div className="flex gap-3"><input type="text" readOnly value={shareUrl} className="flex-1 border-2 rounded-xl px-4 py-3 text-xl bg-white font-mono" /><button onClick={handleCopyLink} className="bg-slate-800 text-white px-8 py-3 rounded-xl font-bold text-lg hover:bg-slate-700">複製</button></div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className={`bg-slate-50 p-4 rounded border ${initialData ? "opacity-70 pointer-events-none" : ""}`}>
              <label className="block text-lg font-bold text-slate-700 mb-2">卡片編號 (ID)</label>
              <div className="flex gap-2 items-center">
                <select className="border rounded p-3 bg-white flex-1 text-lg" value={formData.series} onChange={(e) => setFormData({ ...formData, series: e.target.value })}>{CARD_SERIES_OPTIONS.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}</select>
                <span className="font-bold text-slate-400">-</span>
                <input type="text" placeholder="001" required={!initialData} className="border rounded p-3 flex-1 text-lg" value={formData.number} onChange={(e) => setFormData({ ...formData, number: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="block text-lg font-bold text-slate-700 mb-2">卡片名稱</label>
              <input type="text" required className="w-full border rounded p-3 text-lg" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-lg font-bold text-slate-700 mb-2">種類</label>
                <select className="w-full border rounded p-3 text-lg" value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })}>{Object.values(CARD_TYPES).map((t) => (<option key={t} value={t}>{t}</option>))}</select>
              </div>
              <div>
                <label className="block text-lg font-bold text-slate-700 mb-2">顏色</label>
                <select className="w-full border rounded p-3 text-lg" value={formData.color} onChange={(e) => setFormData({ ...formData, color: e.target.value })}>{Object.values(CARD_COLORS).map((c) => (<option key={c} value={c}>{c}</option>))}</select>
              </div>
            </div>
            {formData.type === CARD_TYPES.COOKIE && (
              <div>
                <label className="block text-lg font-bold text-slate-700 mb-2">等級 (Level)</label>
                <select className="w-full border rounded p-3 text-lg" value={formData.level} onChange={(e) => setFormData({ ...formData, level: e.target.value })}>{Object.values(CARD_LEVELS).map((lvl) => (<option key={lvl} value={lvl}>{lvl}</option>))}</select>
              </div>
            )}
            <div className="bg-slate-50 p-4 rounded-lg border">
                <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isFlip} onChange={(e) => setFormData({ ...formData, isFlip: e.target.checked })} /><span>FLIP</span></label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isExtra} onChange={(e) => setFormData({ ...formData, isExtra: e.target.checked })} /><span>Extra Deck</span></label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isAncient} onChange={(e) => setFormData({ ...formData, isAncient: e.target.checked })} /><span>上古餅乾</span></label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isDragon} onChange={(e) => setFormData({ ...formData, isDragon: e.target.checked })} /><span>龍族</span></label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isBeast} onChange={(e) => setFormData({ ...formData, isBeast: e.target.checked })} /><span>野獸餅乾</span></label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-base"><input type="checkbox" className="w-5 h-5" checked={formData.isSoulJam} onChange={(e) => setFormData({ ...formData, isSoulJam: e.target.checked })} /><span>靈魂果醬</span></label>
                </div>
                <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-y-3 gap-x-4">
                    <label className="flex items-center gap-2 cursor-pointer text-red-600 font-bold text-lg"><input type="checkbox" className="w-6 h-6 accent-red-600" checked={formData.isForbidden} onChange={(e) => setFormData({ ...formData, isForbidden: e.target.checked })} /><span>🚫 禁止卡</span></label>
                    <label className="flex items-center gap-2 cursor-pointer text-orange-600 font-bold text-lg"><input type="checkbox" className="w-6 h-6 accent-orange-600" checked={formData.isLimitOne} onChange={(e) => setFormData({ ...formData, isLimitOne: e.target.checked })} /><span>⚠️ 限制卡</span></label>
                </div>
            </div>
          </div>
          <div className="space-y-6">
            <div>
              <label className="block text-lg font-bold text-slate-700 mb-2">圖片 {initialData && <span className="text-sm font-normal text-gray-500">(不更換則維持原圖)</span>}</label>
              <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 relative h-80 flex items-center justify-center bg-slate-100">
                {previewUrl ? <img src={previewUrl} className="absolute inset-0 w-full h-full object-contain" /> : <div className="text-slate-400 flex flex-col items-center gap-2"><ImageIcon size={64} /><span className="text-lg font-bold">點擊上傳圖片</span></div>}
                <input type="file" accept="image/*" onChange={handleFileChange} className="absolute inset-0 opacity-0 cursor-pointer" />
              </div>
            </div>
            <button type="submit" disabled={isProcessing} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold disabled:opacity-50 text-xl shadow-lg hover:bg-blue-700 transition-colors">{isProcessing ? "處理中..." : initialData ? "更新卡片資訊" : "確認上傳並同步"}</button>
          </div>
        </form>
      </div>
    </div>
